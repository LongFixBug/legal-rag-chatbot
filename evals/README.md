...
- `answer`: verifies the final answer includes required legal facts and avoids
  known bad phrases.
- `calculation`: verifies deterministic legal calculations such as tax formulas.
...
```bash
EVAL_GROUPS=tax.inheritance,tax.prize uv run pytest tests/test_quality_eval.py -q
```
